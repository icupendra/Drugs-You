...
private static final int DATABASE_VERSION = 1;
	...
	public	void AddtoFavorite(Pojo pj)
	{
		...
		values.put(KEY_SID, pj.getSId());
		values.put(KEY_STITLE, pj.getSTitle());
		values.put(KEY_SDES, pj.getSDes());
		...
		public List<Pojo> getAllData()
		{
			...
			if (cursor.moveToFirst()) 
			{
				do {
					...
					contact.setSTitle(cursor.getString(2));
					contact.setSDes(cursor.getString(3));
	...
	//getting single row
		...
		public List<Pojo> getFavRow(String id)
		{
			List<Pojo> dataList = new ArrayList<>();
			// Select All Query
			String selectQuery = "SELECT  * FROM " + TABLE_NAME +" WHERE sid="+id;
	...
	//for remove favorite
		...
		{
		    SQLiteDatabase db = this.getWritableDatabase();
		    db.delete(TABLE_NAME, KEY_SID + " = ?",
		            new String[] { String.valueOf(contact.getSId()) });
		    db.close();
		}
